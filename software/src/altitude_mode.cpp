...
void read_receiver(void) {
    ...
    if (valid) {
        ...
        const float DEADBAND_THROTTLE = 50.0f; // ±50 units for throttle
        ...
        const float CENTER_THROTTLE = 500.0f; // No altitude change
        ...
        if (fabs(temp_throttle - CENTER_THROTTLE) < DEADBAND_THROTTLE) {
            temp_throttle = CENTER_THROTTLE;
        }
...
float AltitudeToF, AltitudeToFStartUp;
float AccZInertial,AccZCalibration = 0;
...
float PRateRoll=1 ; float PRatePitch=PRateRoll; float PRateYaw=45;
float IRateRoll=3.5 ; float IRatePitch=IRateRoll; float IRateYaw=10;
float DRateRoll=0.05 ; float DRatePitch=DRateRoll; float DRateYaw=0.01;
...
void setup(){
    ...
    /* Fill the platform structure with customer's implementation. For this example, only the I2C address is used.*/
    Dev.platform.address = LIDAR_ADDR;
	...
	if (status)
	{
        ...
        printf("VL53L8CX ULD Loading failed with error code %d\n", status);
        while (1)
        {
            tight_loop_contents(); // Wait here if initialization fails
        }
...
void main_loop() {
    ...
    DesiredVelocityVertical = 0.01 * (ReceiverValue[2] - 500); // Scale throttle to vertical velocity (0-1000 throttle maps to -5 to +5 cm/s)
    ...
    if(debug_counter++ > 100) {
        ...
        printf("Altitude: %.1f cm, Velocity: %.1f cm/s\n", AltitudeKalman, VelocityVerticalKalman);
        printf("AltitudeToF: %.1f cm, Desired Velocity: %.1f cm/s, Zaccel: %.1f\n", 
               AltitudeToF, DesiredVelocityVertical, AccZInertial);
        debug_counter = 0;
    }
