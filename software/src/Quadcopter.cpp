...
ROTATE                                 Ended startup, ready to arm
...
##########################################################################################################################*/
#define AHR_USE AHR_USE_MAHONY  // Or AHR_USE_MAHONY_BF
#include <madflight/madflight.h>
#include <algorithm>
...
const float maxRollRate =  20;   //60.0;  // Max roll rate in deg/sec for rate mode
const float maxPitchRate = 20;   //60.0; // Max pitch rate in deg/sec for rate mode
...
void setup()
{
  ...
  printf("OUT: Setup motors\n");

  // Setup 4 motors for the quadcopter
  out.setupMotor(0, 14, out_hz, min_us, max_us);
  out.setupMotor(1, 24, out_hz, min_us, max_us);
  out.setupMotor(2, 10, out_hz, min_us, max_us);
  out.setupMotor(3, 2, out_hz, min_us, max_us);
  ...
  // rcl simulation: set
  rcl.setMode(RCLSimulation::MANUAL);
}
...
void imu_loop()
{
  ...
  // Simulate RC inputs for testing
  rcl.update(ahr.dt); // Update radio commands from simulation
...
void led_Blink()
{
  ...
  if(out.isArmed()){
    droneLeds.updateBlinkPattern(800, 200);
  }
  else{
    droneLeds.updateBlinkPattern(100, 900);
  }
}
...
void control_Angle(bool zero_integrators)
{
  ...
  integral_roll += error_roll * ahr.dt;
  integral_roll = std::clamp(integral_roll, -i_limit, i_limit); // Saturate integrator to prevent unsafe buildup
  ...
  integral_pitch += error_pitch * ahr.dt;
  integral_pitch = std::clamp(integral_pitch, -i_limit, i_limit); // Saturate integrator to prevent unsafe buildup
  ...
  else
  {
    ...
    integral_yawRate += error_yawRate * ahr.dt;
    integral_yawRate = std::clamp(integral_yawRate, -i_limit, i_limit); // Saturate integrator to prevent unsafe buildup
    float derivative_yawRate = (error_yawRate - error_yawRate_prev) / ahr.dt;
  ...
  printf("PID: %.2f %.2f %.2f\n", PIDroll.PID, PIDpitch.PID, PIDyaw.PID);
}
...
void control_Rate(bool zero_integrators)
{
  ...
  integral_roll += error_roll * ahr.dt;
  integral_roll = std::clamp(integral_roll, -i_limit, i_limit); // Saturate integrator to prevent unsafe buildup
  float derivative_roll = (error_roll - error_roll_prev) / ahr.dt;
  ...
  integral_pitch += error_pitch * ahr.dt;
  integral_pitch = std::clamp(integral_pitch, -i_limit, i_limit); // Saturate integrator to prevent unsafe buildup
  float derivative_pitch = (error_pitch - error_pitch_prev) / ahr.dt;
  ...
  integral_yaw += error_yaw * ahr.dt;
  integral_yaw = std::clamp(integral_yaw, -i_limit, i_limit); // Saturate integrator to prevent unsafe buildup
  float derivative_yaw = (error_yaw - error_yaw_prev) / ahr.dt;
...
void out_KillSwitchAndFailsafe()
{
  ...
  if (!out.isArmed() && rcl.throttle == 0 && rcl.arm && !rcin_arm_prev)
  {
    out.setArmed(true);
    printf("OUT: ARMED");
  }
  ...
  if (out.isArmed() && (!rcl.arm || !rcl.connected()))
  {
    out.setArmed(false);
    if (!rcl.arm)
    {
    ...
    {
      printf("OUT: DISARMED due to lost radio connection");
    }
  }
